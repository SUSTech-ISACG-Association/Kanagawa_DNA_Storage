...
/**
 * @brief Initialize Reed-Solomon encoder/decoder
 */
ReedSolomon::ReedSolomon(uint8_t msg_length, uint8_t ecc_length) : msg_length(msg_length), ecc_length(ecc_length)
{
...
ReedSolomon::~ReedSolomon() { memory = NULL; }

void ReedSolomon::EncodeBlock(const void *src, void *dst)
{
    static uint8_t generator_cache[256] = {0};
...
int ReedSolomon::DecodeBlock(const void *src, const void *ecc, void *dst, uint8_t *erase_pos, size_t erase_count)
{
    ...
    bool ok = FindErrors(reloc, src_len);
    if (!ok)
        return 1;
    if (err->length == 0)
        return 1;
